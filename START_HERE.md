...
このフォルダは、**FFXIV Collect所持率表示を追加した更新ファイル** です。ブランドUIの復元、登録不要検索、追加フィルターを保持したまま、マウント・ミニオン・アチーブメントの各カードへ所持率を追加しています。Gitの履歴とリモート接続情報を保つため、既存の `eorzean-collection` フォルダは削除しません。
